...
generation seed when calculating thed empirical or the 
asymptotic power based on the exact variance formula
to get reproducible results.
We illustrate these features next. 
This example is based on $B=\Sexpr{10}$
simulation repplicates.
<<ex1b>>=
set.seed(123)
...
power calculations for this combination of paramaters. This is
...
\par
The session information is provided in Table~\ref{tab:session}
